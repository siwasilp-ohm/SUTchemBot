...
-- =====================================================
-- DEMO DATA v2: UTF-8 Clean Version
...
-- =====================================================
-- PHASE 0: Clean previous demo data
-- =====================================================
DELETE FROM disposal_bin WHERE txn_id IN (SELECT id FROM chemical_transactions WHERE txn_number LIKE 'TXN-DEMO-%');
DELETE FROM chemical_transactions WHERE txn_number LIKE 'TXN-DEMO-%';

-- =====================================================
-- PHASE 1: USE transactions
-- =====================================================
...
('TXN-DEMO-USE-020','stock',15,7,'F01121A5900001','use',12,12,12,50.00,'mL',2450.00,'ใช้ EtOH ล้างอุปกรณ์','Equipment Cleaning','completed','2025-12-01 10:00:00'),
('TXN-DEMO-USE-021','stock',16,8,'F01121A5900002','use',12,12,12,30.00,'mL',2470.00,'ใช้ IPA ทำความสะอาด','Cleaning','completed','2026-01-18 11:00:00'),
...
('TXN-DEMO-USE-027','stock',3,2983,'320F6600000003','use',14,14,14,50.00,'mL',2450.00,'ใช้ H2O2 ฟอกตัวอย่าง','Bleaching','completed','2026-01-25 15:00:00'),
('TXN-DEMO-USE-028','stock',1,18,'320F6600000001','use',5,5,5,5.00,'kg',20.00,'ทดสอบ CO2 ในห้องปฏิบัติการ','Safety Test','completed','2026-01-30 09:00:00'),
...
('TXN-DEMO-BRW-012','stock',4,2983,'320F6600000004','borrow',1,10,10,500.00,'mL',2000.00,'ยืม H2O2 สำหรับ oxidation experiment','Oxidation Study',1,'completed',5,'2026-01-20 08:30:00','2026-02-05','2026-01-20 08:00:00');

-- =====================================================
-- PHASE 3: RETURN transactions
-- =====================================================
INSERT INTO chemical_transactions
(txn_number, source_type, source_id, chemical_id, barcode, txn_type,
 from_user_id, to_user_id, initiated_by, quantity, unit, balance_after,
...
('TXN-DEMO-RET-007','stock',4,2983,'320F6600000004','return',10,1,10,450.00,'mL',2450.00,'คืน H2O2 ใช้ไป 50mL','completed','2026-02-03','partially_used','2026-02-03 14:00:00');

-- Link returns to borrows
...
('TXN-DEMO-TRF-004','stock',7,2983,'320F6600000007','transfer',1,11,1,500.00,'mL',2000.00,'แจกจ่าย H2O2 ให้ lab2','Supply Distribution','ไอที','เคมี','completed',1,'2026-01-12 09:00:00','2026-01-12 08:30:00'),
...
('TXN-DEMO-TRF-007','stock',8,2983,'320F6600000008','transfer',1,13,1,1000.00,'mL',1500.00,'แจกจ่าย H2O2 ให้ lab3','Supply Distribution','ไอที','ชีววิทยา','completed',1,'2026-02-05 09:00:00','2026-02-05 08:30:00');

-- =====================================================
-- PHASE 5: DISPOSE transactions
-- =====================================================
INSERT INTO chemical_transactions
(txn_number, source_type, source_id, chemical_id, barcode, txn_type,
 from_user_id, to_user_id, initiated_by, quantity, unit, balance_after,
...
('TXN-DEMO-DSP-003','stock',3,2983,'320F6600000003','dispose',7,NULL,7,2450.00,'mL',0.00,'จำหน่าย H2O2 เก่า','pending','สารเก่า ต้องการแทนที่ด้วย lot ใหม่','return_to_vendor',NULL,'2026-02-13 09:00:00');

-- =====================================================
-- PHASE 6: Disposal bin
-- =====================================================
...
('container',7,3,'CHEM-20240214-003B','กรดแอซิติก (Acetic Acid)',15.00,'mL',10,'เหลือปริมาณน้อยเกินไป ไม่คุ้มเก็บ','neutralization','นักวิจัย สอง','เคมี','completed',5,'2026-01-10 16:30:00','2026-01-11 10:00:00','2026-01-10 16:00:00'),
('container',5,2,'CHEM-20240214-002B','กรดซัลฟิวริก (Sulfuric Acid)',45.00,'mL',11,'สารเหลือน้อยและใกล้หมดอายุ','neutralization','นักวิจัย สาม','เคมี','completed',6,'2026-02-01 15:00:00','2026-02-02 09:00:00','2026-02-01 14:30:00'),
('stock',3,NULL,'320F6600000003','Hydrogen peroxide',2450.00,'mL',7,'สารเก่า ต้องการแทนที่ด้วย lot ใหม่','return_to_vendor','ผู้จัดการ สาม','ชีววิทยา','pending',NULL,NULL,NULL,'2026-02-13 09:00:00');
...
(6,'used',10,-200.00,2000.00,'ทำความสะอาด column','2025-12-08 13:30:00'),
...
(2,'transferred',5,-50.00,170.00,'โอนจาก user1 ไป user2','2025-12-05 08:30:00'),
(9,'transferred',6,-100.00,390.00,'โอนจาก user3 ไป user4','2025-12-20 09:30:00'),
(12,'transferred',5,-100.00,250.00,'โอนจาก user2 ไป lab4','2026-01-25 10:30:00'),
...
-- =====================================================
-- PHASE 10: Update disposed containers
-- =====================================================
...
('expiry','critical','สารเคมีใกล้หมดอายุ','กรดแอซิติก (ACETIC-2023-010) เหลืออายุ 5 วัน',10,3,4,0,1,'2025-10-01 08:00:00'),
('low_stock','warning','สต็อกใกล้หมด','Acetone (ACETONE-2023-020) คงเหลือ 3.6%',9,6,11,0,1,'2025-10-02 09:00:00'),
('low_stock','warning','สต็อกใกล้หมด','Isopropanol (IPA-2023-015) คงเหลือ 3.2%',11,8,15,0,1,'2025-10-03 09:00:00'),
('overdue_borrow','warning','ยืมเกินกำหนด','EtOH 300 mL กำหนดคืน 20 วันก่อน',10,7,14,0,1,'2025-10-05 08:00:00'),
('overdue_borrow','warning','ยืมเกินกำหนด','Acetone 100 mL กำหนดคืน 15 วันก่อน',12,6,12,0,1,'2025-10-06 08:00:00'),
('borrow_request','info','คำขอยืมใหม่','นักวิจัย สี่ ขอยืม HCl 100 mL',9,1,1,1,1,'2025-10-10 10:00:00'),
('borrow_request','info','คำขอยืมใหม่','นักวิจัย หก ขอยืม NaOH 50 g',11,4,8,1,1,'2025-10-12 10:00:00'),
('expiry','critical','สารเคมีหมดอายุแล้ว','HCl (CHEM-20240214-001C) หมดอายุ 24 ชั่วโมง',10,1,3,1,1,'2025-10-15 08:00:00'),
('low_stock','warning','สต็อกใกล้หมด','ViSafe Red (BTL-0007) คงเหลือ 60%',13,NULL,NULL,0,1,'2025-12-01 09:00:00'),
('low_stock','warning','สต็อกใกล้หมด','100 bp DNA Ladder (BTL-0008) คงเหลือ 70%',13,NULL,NULL,0,1,'2025-12-05 09:00:00'),
...
SELECT 'Disposal bin:' as label, COUNT(*) as total FROM disposal_bin;
SELECT 'Alerts:' as label, COUNT(*) as total FROM alerts;
